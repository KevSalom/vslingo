...
- La verificación desplegada encontró que Render servía `app.webmanifest` como `binary/octet-stream`; se añadió el tipo explícito al enlace y una regla `Content-Type: application/manifest+json` al Blueprint, junto con `SITE_URL` y `PUBLIC_API_URL` de producción. Esta corrección requiere un nuevo deploy para cerrar su comprobación HTTP.
- Clerk carga Google y código por correo, pero el modal desplegado todavía muestra `Development mode`; la vuelta real no cierra el gate hasta sustituir las claves `pk_test_`/`sk_test_`/PEM por una instancia Clerk de producción y repetir correo, Google, logout y segundo dispositivo.
