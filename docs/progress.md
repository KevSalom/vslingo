...
- **Bugfix post-T10 (2026-07-27):** Voice Studio — etiquetas de burbuja fijas (`Tú` / `VSLingo`); ondas mic/VAD (gate suave `isSpeech`, reset al fin); ganancia visual del agente; PTT anima barras en vivo vía `AudioRecorder.onFrameLevel` (antes nivel fijo 0.55); al cambiar solo el proveedor TTS se conserva el historial de conversación en UI (alineado al backend, que solo limpia al cambiar escenario).
