...
- El usuario `kwebtrader` conservaba un checkout `APPROVAL_PENDING` cuya URL de aprobación ya terminaba en el error genérico de PayPal. La recuperación ahora es transparente: el mismo botón «Continuar en PayPal» consulta primero el estado real, nunca reemplaza una suscripción aprobada o activa y, sólo si continúa pendiente, cancela el intento anterior y crea uno nuevo de forma transaccional e idempotente. Falta comprobar el flujo completo después del deploy público.
