...
- Backend completo: lock válido, Ruff y mypy estricto en verde; **192 tests pytest** pasaron. Frontend completo: Astro check con **0 errores y 0 warnings** (5 hints heredados), **145 tests Vitest** pasaron y el build generó siete rutas. Chrome y Edge ejecutaron **11/11 recorridos E2E** cada uno, incluido registro del service worker y precarga de la pantalla offline; el proceso Playwright requirió interrupción manual después de imprimir todos los casos aprobados porque su servidor preview no terminó en Windows.
