...
- La sesión autenticada reveló que el middleware exigía bearer token al preflight CORS de `/api/history/*`, por lo que Escritura y Videos mostraban un error al abrir el historial. Se excluyeron las solicitudes `OPTIONS` de autenticación y se añadió una regresión que reproduce `Authorization, Content-Type`; falta comprobar el nuevo deploy público.
