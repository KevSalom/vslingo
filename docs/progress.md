...
- Backend completo: lock válido, Ruff y mypy estricto en verde; **199 tests pytest** pasaron. Frontend completo: Astro check con **0 errores y 0 warnings** (5 hints heredados), **149 tests Vitest** pasaron y el build generó siete rutas. Chrome y Edge ejecutaron **11/11 recorridos E2E** cada uno, incluido registro del service worker y precarga de la pantalla offline; el proceso Playwright requirió interrupción manual después de imprimir todos los casos aprobados porque su servidor preview no terminó en Windows.
...
- El mismo smoke reveló que un espacio terminal podía hacer que la respuesta estructurada no coincidiera literalmente con `original_text`. Frontend y backend ahora normalizan únicamente el espacio circundante antes de validar, medir y enviar, conservando saltos de línea internos; las regresiones cubren espacios, tabulaciones y finales CRLF. Falta comprobar el nuevo deploy público.
- Un checkout PayPal Sandbox real fue aprobado con una cuenta Personal y PayPal entregó con HTTP exitoso `BILLING.SUBSCRIPTION.CREATED`, `BILLING.SUBSCRIPTION.ACTIVATED` y `PAYMENT.SALE.COMPLETED` por US$2,99. El ensayo reveló que el panel de PayPal no distingue un evento aplicado de uno rechazado internamente: la suscripción quedó `active` sin periodo ni cuota mensual. El código conserva el webhook como respaldo, pero el retorno autenticado ahora consulta servidor a servidor la suscripción y sus transacciones, valida plan, importe y moneda, y aplica el pago de forma idempotente; la UI no etiqueta `active` como pagado hasta que exista `access_ends_at` y ofrece reintento explícito. Falta comprobar este arreglo en el deploy y revisar el `error_code` del evento original, en particular la correspondencia exacta de `PAYPAL_MERCHANT_ID`.
- El usuario `kwebtrader` conservaba un checkout `APPROVAL_PENDING` cuya URL de aprobación ya terminaba en el error genérico de PayPal. La primera recuperación desplegada confirmó que PayPal devolvía un rechazo HTTP 4xx antes de sustituir el intento local. El flujo transparente ahora distingue las respuestas del proveedor: si el recurso pendiente ya no existe (`404`) o PayPal rechaza su cancelación por reglas de estado (`422`), reconsulta para cerrar la carrera de aprobación y sólo entonces sustituye localmente el intento; nunca reemplaza una suscripción aprobada o activa. Falta comprobar el flujo completo después del nuevo deploy público.
- Esta evidencia automatizada no completa F7. Continúan pendientes PWA y micrófono en Chrome/Edge/Android/iPhone físicos, Clerk de producción por correo/Google, cierre del ensayo PayPal Sandbox con cuota conciliada, una compra Live mínima autorizada con cancelación/devolución, restauración cifrada en infraestructura real, y aprobación de dominio, soporte y textos legales.
